...
<script>
  ...
  export let details;
  export let color =
    tags.length == 1 || (max_tags == 1 && tags.length > 1)
      ? tags[0].color
      : "primary";
  ...
  $: textColor =
    color == "primary" || color == "tertiary" ? "secondary" : "primary";
  $: clr = {
...
>
  ...
  <div class="flex flex-col px-5 py-4 {!!details ? 'mt-6 md:mt-8' : ''}">
    ...
    <p
      class="body-sm order-4 max-w-[65ch] pr-3 text-{clr.text} whitespace-pre-line"
    >
      <slot name="about" />
    </p>
  </div>
  ...
  >
    ...
    #242331
            ...
            hover ? "opacity: 25% !important" : "opacity: 100 !important"
          }`
        : "display: hidden;"}
    />
...
<div
  class="to-tertiary to-primary to-secondary to-secondary-light to-orange to-green to-purple"
/>
